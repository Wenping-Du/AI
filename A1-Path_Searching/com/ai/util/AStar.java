...
public class AStar {
     ...
     * @param col map col
     */
    public String aStarSearch_e(int[][] map, int start_r, int start_c, int end_r, int end_c, int row, int col){
...
//            System.out.println("cost: " + n.cost);
            ...
            for(int t = 0; t < 4; t ++) {
                ...
                if(x >= 0 && y >= 0 && x < row && y < col && map[x][y] != 0) {
                    ...
                    if(!n.getPaths().contains(thisPath)){
                        ...
                        int e_cost = (int) Math.sqrt(Math.pow(x - n.row, 2) + Math.pow(y - n.col, 2 ));
                        newNode.setCost(n.getCost() + nodeCost + e_cost);
     ...
     * @return
     */
    public String aStarSearch_m(int[][] map, int start_r, int start_c, int end_r, int end_c, int row, int col){
        Node initialNode = new Node(start_r - 1, start_c - 1);
        initialNode.setPaths("0,0");
        queue.clear();
        queue.add(initialNode);
            ...
            for(int t = 0; t < 4; t ++) {
                ...
                if(x >= 0 && y >= 0 && x < row && y < col && map[x][y] != 0) {
                    ...
                    if(!n.getPaths().contains(thisPath)){
                        ...
                        int m_cost = Math.abs(x - n.row) + Math.abs(y - n.col);
                        Node newNode = new Node(x, y);
                        newNode.setCost(n.getCost() + nodeCost + m_cost);
                        newNode.setPaths(n.getPaths() + "---" + x + "," + y);
                        newNode.setTime(System.currentTimeMillis());
                        queue.offer(newNode);
                    }
